...
// Point — место на карте в момент времени. Координаты в той же системе, что и
// в реплее: карта занимает примерно от 64 до 192 по обеим осям.
type Point struct {
	T    int
	X, Y float64
}
...
type Player struct {
	...
	// FromScoreboard — основные числа пришли из итоговой таблицы Valve.
	// Их незачем перекрывать своим разбором: это те же величины, только из
	// первых рук. Свой разбор нужен там, где таблица молчит, и как страховка,
	// если таблицы нет вовсе.
	FromScoreboard bool
	...
	// Path — где герой был: точка на каждые несколько секунд. DeathsAt —
	// где он умирал. Обе заполняются разбором реплея и нужны всему, что
